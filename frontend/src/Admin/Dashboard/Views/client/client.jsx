        ...
        <Fragment>
            <div className="parentHomeDashboard">
                ...
                <div className="TitleDashboard formations">
                    <h1>Clients</h1>
                    <div>
                        <button className="btnSeeMoreServices" title="Ajouter Client" onClick={openAddClient}>
                            <i className='bx bx-plus-medical' ></i>
                        </button>
                        <button className="btnSeeMoreServices trash" title="Poubelle" onClick={openTrashedClients}>
                ...
                <div className="rowControlsFormation">
                    ...
                    <form className="formSearchFormation" onSubmit={(e) => e.preventDefault()}>
                        <input type="text" placeholder="Chercher (Nom, Prénom, Email, Ville) ..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                        <button type="submit">
                    ...
                    <table border='1' cellSpacing='0'>
                        ...
                        <tbody>
                            {sortedClients.map((client, index) => (
                                <tr key={index}>
                                    <td>{client.id}</td>
                                    <td><span className="spanNomUppercase">{client.nom}</span></td>
                                    <td>{client.prenom}</td>
                                    <td>{client.email}</td>
                                    <td>{client.ville}</td>
                                    ...
                                    <td className="tdActions">
                                        <div>
                                            <button className='btnEdit' onClick={() => openEditClient(client.id)}>
                                                <i className='bx bxs-edit-alt'></i>
                                            </button>
                                            <button className='btnDelete' onClick={() => deleteClient(client.id)}>
                                                <i className='bx bxs-trash-alt' ></i>
                                            </button>
                                        </div>
                                    </td>
                ...
                {toggleAddClient && <ModalAddClient onClose={closeAddClient} setResponseMessage={setResponseMessage}/>}
                {toggleEditClient && <ModalEditClient clientId={editClientId} onClose={closeEditClient} setResponseMessage={setResponseMessage}/>}
                {togglePoubelle && <ModalTrashedClients onClose={closeTrashedClients}  setResponseMessage={setResponseMessage} />}
            </div>
        </Fragment>
    )
}

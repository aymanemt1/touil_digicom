...
export default function FormationDashboard() {
  ...
  function deleteFormation(id) {
    if (window.confirm("Êtes-vous sûr de vouloir supprimer cette formation?")) {
      axios
        .delete(`${apiUrl}/api/formations/${id}`)
        .then((response) => {
      ...
      <div className="parentHomeDashboard">
              ...
              {sortedFormations.map((item, index) => (
                <tr key={index}>
                  ...
                  <td id="tdImgFormations">
                    <img
                      src={`${apiUrl}/storage/formations/cover/${item.cover}`}
                    />
                  </td>
                  <td id="tdImgFormations">
                    <img
                      src={`${apiUrl}/storage/formations/affiche/${item.affiche}`}
                    />
                  </td>
                  <td>
                    <h4>{item.titre_fr}</h4>
                  ...
                  <td className="tdActions">
                    <div>
                      <button
                        className="btnEdit"
                        onClick={() => openEditFormation(item.id)}
                      >
        ...
        {toggleEditFormation && (
          <ModalEditFormation
            ...
            onClose={closeEditFormation}
            setResponseMessage={setResponseMessage}
          />
        )}
        {togglePoubelle && (
          <ModalTrashedFormation
            onClose={closeTrashedFormation}
            setResponseMessage={setResponseMessage}
          />
        )}
      </div>

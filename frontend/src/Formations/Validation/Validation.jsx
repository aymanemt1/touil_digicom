...
export const Validation = ({ formData }) => {
  ...
  const validation = Translate.validation.find((lang) => lang.id == langue);
  ...
  const getFormationDates = async (formationId) => {
    try {
      const response = await axios.get(
        `${apiUrl}/api/GetformationDate/${formationId}`
      );
      ...
      if (reservationResponse.status === 200) {
        ...
        try {
          const emailResponse = await axios.post(
            `${apiUrl}/api/sendmail-reservaion`,
            updatedFormData
          );
  ...
  const ticketBtn = async () => {
    ...
    try {
      const response = await axios.post(`${apiUrl}/api/ticket`, ticketData, {
        responseType: "blob",
      });
  ...
  return (
    <div className="validationParent">
      <div class="validation" id={reserved ? "removingbg" : ""}>
        ...
        >
          <div className="formdate">
            <select
              ...
              value={validationData.date_validation}
          ...
          <div className="formtime">
            <select
              ...
              value={validationData.time_validation}
              onChange={handleChange}
            >
              <option value="" selected disabled>
        ...
        <div
          className={`validationEntit ${Errors ? "afterErrors" : ""}`}
          id={reserved ? "Afterreserved" : ""}
        >
          {reserved ? (
            <>
              <h1 className="msgreservation">{validation.remerci}</h1>
              <div className="btnsAfterReserved">
                <button className="backServicebtn" onClick={handlReservice}>
                  {validation.back}
                </button>
                <button className="ticketBtn" onClick={ticketBtn}>
                  {validation.ticket}
                </button>
              </div>
            </>
